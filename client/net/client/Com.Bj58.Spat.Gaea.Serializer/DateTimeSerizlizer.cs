...
{
    internal class DateTimeSerizlizer : SerializerBase
    {
        ...
        private byte[] ConvertToBinary(DateTime dt)
        {
            return Convert.ToInt64((dt - DateTime.Parse("1970-1-1 00:00:00")).TotalMilliseconds).GetBytes();
        }
        private DateTime GetDateTime(byte[] buffer)
        {
            return DateTime.Parse("1970-1-1 00:00:00").AddMilliseconds(buffer.ToInt64(0));
        }
    }
}
